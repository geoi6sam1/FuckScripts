...
if (GM_getValue('setting_hide_br')) {
    main.hidebr();
};
...
<label class="setting-label">隐藏底部最近的搜索<input id="hide_br" ${GM_getValue('setting_hide_br') ? 'checked' : ''} type="checkbox" class="switch-btn switch-btn-animbg" /></label>
<label class="setting-label">隐藏最新相关信息(资讯)<input id="hide_zx" ${GM_getValue('setting_hide_zx') ? 'checked' : ''} type="checkbox" class="switch-btn switch-btn-animbg" /></label>
    ...
    document.querySelector('#hide_br').addEventListener('change', (e) => {
        GM_setValue('setting_hide_br', e.target.checked);
    });
